...
<div class="row align-items-center mb-4">
    <div class="col">
        ...
        <h1><i class="bi bi-box-arrow-in-right text-success"></i> Contas a Receber</h1>
    </div>
    <div class="col-auto">
        <form class="d-flex gap-2" method="GET" action="<?= URL_BASE ?>/financeiro/receber">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($contas as $conta): ?>
                            <?php 
                            $vencido = false;
                            if ($conta['status'] == 'pendente' && strtotime($conta['data_vencimento']) < strtotime('today')) {
                                $vencido = true;
                            }
                            ?>
                            <tr class="<?= $vencido ? 'table-danger' : '' ?>">
                                ...
                                <td><?= $conta['cliente_nome'] ?: 'Cliente Padrão' ?></td>
                                <td>#<?= str_pad($conta['venda_id'], 5, '0', STR_PAD_LEFT) ?></td>
                                <td><?= $conta['numero_parcela'] ?>/<?= $conta['total_parcelas'] ?></td>
                                <td>R$ <?= number_format($conta['valor'], 2, ',', '.') ?></td>
                                <td>
                                    <?php if ($conta['status'] === 'pago'): ?>
                                        <span class="badge bg-success">Recebido</span><br>
                                ...
                                <td class="text-end">
                                    <a href="<?= URL_BASE ?>/vendas/detalhes/<?= $conta['venda_id'] ?>" class="btn btn-sm btn-outline-primary" title="Ver Venda Original">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    
                                    <?php if ($conta['status'] === 'pago'): ?>
                                        <a href="<?= URL_BASE ?>/financeiro/estornarReceber/<?= $conta['id'] ?>" class="btn btn-sm btn-outline-danger ms-1" title="Estornar Recebimento" onclick="return confirm('Desfazer baixa?');">
                                            <i class="bi bi-arrow-counterclockwise"></i>
                                        </a>
                                    <?php else: ?>
                                        <button type="button" class="btn btn-sm btn-success ms-1" data-bs-toggle="modal" data-bs-target="#modalReceber<?= $conta['id'] ?>" title="Registrar Recebimento">
                                            <i class="bi bi-check-lg"></i> Receber
                                        </button>
                                        
                                        <!-- Modal de Recebimento -->
                                        <div class="modal fade" id="modalReceber<?= $conta['id'] ?>" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content text-start border-success border-top border-4">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title fw-bold text-success">Confirmar Recebimento</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <form action="<?= URL_BASE ?>/financeiro/baixarReceber/<?= $conta['id'] ?>" method="POST">
                                                        <div class="modal-body">
                                                            <p>Confirmando o recebimento da <strong><?= $conta['numero_parcela'] ?>ª Parcela</strong> no valor de <strong>R$ <?= number_format($conta['valor'], 2, ',', '.') ?></strong>.</p>
